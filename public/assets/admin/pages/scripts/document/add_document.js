...
$(document).ready(function () {
    ...
    $('#payed_option').on('switchChange.bootstrapSwitch', function () {
        if($(this).prop('checked') == true) {
            $('.document-price-div').slideDown('slow');
        } else {
            $('.document-price-div').slideUp('slow');
        }
    });

    $('.add-document').click(function() {
        var document_name = $('.document-name').val();
        var document_price = $('.document-price').val();
        var document_template = CKEDITOR.instances['document-template'].getData();

        $.ajax({
            type: "GET",
            url: 'add_ajax',
            data: {document_name: document_name, document_price: document_price, document_template: document_template},
            success: function( data ) {
                if(data['success'] == true) {
                    toastr['success']("Document Added Successfully!", "Success")

                    $('.document-name').val('');
                    $('.document-price').val('0');
                    CKEDITOR.instances['document-template'].setData('');
                    $('.document-name').focus();

                } else {
                    toastr['warning']("Document not Added Successfully!", "Warning")
                }
            }
        });
